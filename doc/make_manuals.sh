...
for available_language in `ls | grep "^cinecutie_cv_manual_.*\.texi$" | sed "s+cinecutie_cv_manual_++g" | sed "s+\.texi++g"`;
do
...
then
	language=$1
	# If there is no texinfo cinecutie CV manual in that language
	if [ ! -f cinecutie_cv_manual_${language}.texi ]
	then
...
rm -rf cinecutie_cv_manual_${language}.{aux,cp,cps,fn,ky,log,pg,toc,tp,vr}
texi2pdf -q --pdf cinecutie_cv_manual_${language}.texi
rm -rf cinecutie_cv_manual_${language}.{aux,cp,cps,fn,ky,log,pg,toc,tp,vr}

echo "------- Making HTML manual (${language}) -------"
texi2html --nosec-nav cinecutie_cv_manual_${language}.texi
...
# website :
texi2html --nosec-nav --no-menu --split chapter cinecutie_cv_manual_${language}.texi
...
makeinfo --plaintext --no-headers cinecutie_cv_manual_${language}.texi -o cinecutie_cv_manual_${language}.txt
rm -rf manual_images_${language}/*.txt
...
then
	...
	${makeinfo_cvs} --docbook --no-number-sections --no-warn cinecutie_cv_manual_${language}.texi -o cinecutie_cv_manual_${language}.xml
	First_line_to_delete=`cat cinecutie_cv_manual_${language}.xml | grep -n "<chapter label=\"\" xreflabel=\"Cinelerra CV Manual\" id=\"Top\">" | sed "+s+:.*++g"`
	if [ -n "${First_line_to_delete}" ]
	then
	      Last_line_to_delete=`expr ${First_line_to_delete} + 3`
	      cat cinecutie_cv_manual_${language}.xml | sed "${First_line_to_delete},${Last_line_to_delete}d" > cinecutie_cv_manual_${language}_temp.xml
	      mv cinecutie_cv_manual_${language}_temp.xml cinecutie_cv_manual_${language}.xml
	fi
fi
