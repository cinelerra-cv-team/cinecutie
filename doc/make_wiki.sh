...
for available_language in `ls | grep "^cinecutie_cv_manual_..\.texi$" | sed "s+.*_++g" | sed "s+\..*++g"`;
do
...
then
	language=$1
	# If there is not texinfo cinecutie CV manual in that language
...
# recreate it
rm -fr cinecutie_cv_${language}
rm -fr cinecutie_cv_${language}.tgz
mkdir cinecutie_cv_${language}

# We generate html pages from the texinfo file.
# Those html page will be converted to the wiki format later.
texi2html --split chapter --nosec-nav --no-menu -prefix cinecutie_cv_${language} cinecutie_cv_manual_${language}.texi

# We go in the directory containing the wiki pages
cd cinecutie_cv_${language}
# For each html page
for html_page in `/bin/ls | grep "\.html$"`;
do
	echo "Converting ${html_page} to cinecutie_cv_${language}/${final_doku_page}"
	...
	cat ${temp_doku_page} | sed "+s+{{\.\.\/\.\/manual_images_\([^/]*\)\/+{{http:\/\/cvs.cinecutie.org\/docs\/manual_images_\1\/+g" | grep -v "^| \[\[#SEC" | sed "+s+\[\[cinecutie_cv_${language}_\([0-9]*\)\.html.SEC[0-9]*.\([^\]]*\)+\[\[english_manual:cinecutie_cv_en_\1\\#\2+g" > ${original_doku_page}
...
rm cinecutie_cv_${language}_25.txt
rm cinecutie_cv_${language}_abt.txt
rm cinecutie_cv_${language}_fot.txt
rm cinecutie_cv_${language}.txt
rm cinecutie_cv_${language}_toc.txt
