...
int SvgMain::process_realtime(VFrame *input_ptr, VFrame *output_ptr)
{
	...
	if (fh_raw == -1 || force_raw_render) // file does not exist, export it
	{
		...
		{
		// Current Inkscape versions do not support --cinecutie-export-file anymore. This lead to a crash in 
		...
		// this version of cinecutie.

			char filename_png[1024];
			char filename_rgba[1024];
		...
		{
			sprintf(command,
				"inkscape --without-gui --cinecutie-export-file=%s %s",
